...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    const hasOutForDeliveryItem = order.items.some(
      (item) => item.itemStatus === "Out for Delivery",
    );
    if (
      (oldStatus === "Out for Delivery" || hasOutForDeliveryItem) &&
      (status === "Shipped" || status === "Ordered")
    ) {
      return res.status(400).json({
        success: false,
        message:
          "Cannot change status back to Shipped or Ordered once a product is Out for Delivery.",
      });
    }
    ...
    if (status === "Delivered") {
      ...
      // If payment was already made (e.g. Online/Wallet), refund to wallet
      if (order.paymentStatus === "Completed" && order.paymentMethod !== "COD") {
        ...
        wallet.transactions.push({
          ...
          type: "Credit",
          description: `Refund for Admin-Cancelled Order #${order._id}`,
          orderId: order._id,
          date: new Date()
        });
        await wallet.save();
      ...
      // If payment was already made (Online/Wallet/COD-Delivered), refund to wallet
      if (order.paymentStatus === "Completed") {
        ...
        wallet.transactions.push({
          amount: order.totalAmount,
          type: "Credit",
          description: `Refund for Admin-Returned Order #${order._id}`,
          orderId: order._id,
          date: new Date(),
        });
        await wallet.save();
...
exports.approveReturnItem = async (req, res) => {
  try {
    ...
    if (item.itemStatus !== "Return Requested") {
      return res.status(400).json({
        success: false,
        message: `Item is not pending a return, current status is ${item.itemStatus}`,
      });
    }
    ...
    // Refund if they paid! (Completed status implies money collected, regardless of method)
    if (order.paymentStatus === "Completed") {
      let wallet = await Wallet.findOne({ userId: order.userId });
      if (!wallet) {
        wallet = new Wallet({
      ...
      // Refund the exact amount they paid for this item
    ...
    item.itemStatus = "Returned"; // Original code continues here...
    ...
    res.json({
      success: true,
      message: "Return requested approved, stock restored.",
    });
  } catch (err) {
...
exports.rejectReturnItem = async (req, res) => {
  try {
    ...
    if (!comment) {
      return res.status(400).json({
        success: false,
        message: "A reason is required to reject a return.",
      });
    }
