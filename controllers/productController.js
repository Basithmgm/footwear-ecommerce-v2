...
const postAddProduct = async (req, res) => {
  ...
  try {
    ...
    try {
      if (!variants)
        throw new Error("Variants data is missing from request body.");
    ...
    for (const v of finalVariants) {
      ...
      if (v.variantImages.length < 3)
        throw new Error(`At least 3 gallery images required for ${v.color}`);
    }
    ...
    return res
      .status(200)
      .json({ success: true, redirectUrl: "/admin/products" });
  } catch (error) {
...
const postEditProduct = async (req, res) => {
  try {
    console.log(
      "DEBUG: postEditProduct hit. Body keys:",
...
const editVariantSize = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error updating variant size." });
  }
};
...
const setVariantOrderLimit = async (req, res) => {
  try {
    ...
    if (!orderLimit || isNaN(orderLimit) || Number(orderLimit) < 1) {
      return res.status(400).json({
        success: false,
        message: "Invalid order limit. Must be at least 1.",
      });
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Order limit updated successfully!" });
  } catch (error) {
    console.error("Error setting variant order limit:", error);
    return res
      .status(500)
      .json({ success: false, message: "Error updating order limit." });
  }
};
