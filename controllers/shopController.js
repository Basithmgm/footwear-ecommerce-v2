...
const getShop = async (req, res) => {
    try {
        ...
        const { search, sort, category, brand, minPrice, maxPrice, color } = req.query;
        ...
        if (genderFilter) {
            activeCategories = activeCategories.filter(c => c.gender === genderFilter);
        }
        ...
        const validProductQuery = {
            ...
            variants: { $elemMatch: { sizes: { $elemMatch: { status: 'Active', isBlocked: false } } } }
        };
        const categoriesWithProducts = await Product.distinct("category", validProductQuery);
        const categoriesWithProductsStrings = categoriesWithProducts.map(id => id.toString());
        ...
        if (color) {
            ...
            query["variants.color"] = { $in: colorList.map(c => new RegExp(c, 'i')) };
        }
        ...
        switch (sort) {
            ...
            case 'az': sortOption = { productName: 1 }; break;
            case 'za': sortOption = { productName: -1 }; break;
        ...
        const brands = await Product.distinct("brand", { category: { $in: activeCategoryIds }, isDeleted: false, isBlocked: false });
        const colors = await Product.distinct("variants.color", { category: { $in: activeCategoryIds }, isDeleted: false, isBlocked: false });
        ...
        res.render("user/shop", {
            ...
            totalPages,
            totalProducts,
            activeMenu: genderFilter ? (genderFilter === 'Men' ? 'men' : 'women') : 'shop', // Highlight "Men" or "Women" in navbar
        ...
        res.status(500).render("user/shop", {
            pageTitle: "Shop",
            products: [],
            currentPage: 1, totalPages: 1, totalProducts: 0, activeMenu: 'shop',
            error: "Failed to load products",
            search: '', selectedSort: '', selectedCategory: '', selectedBrand: [], selectedColor: [], minPrice: '', maxPrice: '',
            categories: [], brands: [], colors: [], currentQuery: {}
        });
    }
};
    ...
    try {
        ...
        if (!productId.match(/^[0-9a-fA-F]{24}$/)) {
            return res.status(404).render("user/shop", {
                pageTitle: "Shop", products: [], currentPage: 1, totalPages: 1, totalProducts: 0, activeMenu: 'shop',
                search: '', selectedSort: '', selectedCategory: '', selectedBrand: [], selectedColor: [], minPrice: '', maxPrice: '',
                categories: [], brands: [], colors: [], currentQuery: {},
