...
const migrateUsers = async () => {
  try {
    ...
    const usersRaw = await mongoose.connection.db
      .collection("users")
      .find({})
      .toArray();
    ...
    for (const rawUser of usersRaw) {
      ...
      if (rawUser.addresses?.length > 0) {
        console.log(
          `Moving ${rawUser.addresses.length} addresses for ${rawUser.email}`,
        );
      ...
      if (Object.keys(updates).length > 0 || Object.keys(unsets).length > 0) {
        ...
        await mongoose.connection.db
          .collection("users")
          .updateOne({ _id: rawUser._id }, finalUpdate);
        console.log(`Migrated user and addresses: ${rawUser.email}`);
      } else {
        console.log(`User already up to date: ${rawUser.email}`);
      }
    }
